...
def train_start():
    ...
    try:
        ...
        if version_id:
            ...
            update_version_status(version_id, status="training")

        model_path   = os.path.join(upload_dir, f"{uid}_best.pt")

        logger.info(
            "[TRAIN/START] Starting training | version_id=%s | epochs=%d | batch=%d | lr=%s | optimizer=%s | zip=%s",
            version_id, cfg["epochs"], cfg["batch_size"], cfg["lr"], cfg["optimizer"], zip_path
        )
        tr.start_training(graph_data, zip_path, extract_path, cfg, model_path)
        logger.info("[TRAIN/START] Training thread launched | uid=%s | version_id=%s", uid, version_id)
        return jsonify({"status": "success", "message": "Training started", "uid": uid})
...
def playground_test():
    ...
    try:
        version_id = request.form.get("version_id")
        logger.info("[PLAYGROUND] Inference requested | version_id=%s", version_id)
        if not version_id:
            return jsonify({"status": "error", "message": "Version ID is required"}), 400
            
        version = dbh.get_version(version_id)
        if not version:
            return jsonify({"status": "error", "message": "Version not found"}), 404
            
        if version.get("status") != "trained" or not version.get("model_path"):
            return jsonify({"status": "error", "message": "Model is not trained yet. Please train it first."}), 400
        ...
        # Load state dict
        device = next(model.parameters()).device if list(model.parameters()) else torch.device('cpu')
        model.load_state_dict(torch.load(version["model_path"], map_location=device))
